...
def sign(request):
    if request.method == 'POST':
        ...
        if logina:
            ...
            profile_user = profile(user = user , email = email , phone = phone , address = address)
            if user:
                profile_user.save()
...
def check_message(request , pk , name):
    user = request.user
    messagess = saved_message.objects.filter(Q(reciver = user) | Q(sender = user)).order_by('time_stamp')
    return render(request ,'chat-user.html' , {'name':name , 'id': pk,'messages': messagess , 'user':user})

def check_message_seller(request):
    messagess = saved_message.objects.filter(reciver = request.user)
    senders = []
    for sd in messagess:
        senders.append(sd.sender)
    senders = list(set(senders))
    return render(request , 'chat-admin.html' , {'senders':senders})
...
def check_message_seller_mes(request , pk , name):
    ...
    for sd in messagess:
        senders.append(sd.sender)
    senders = list(set(senders))
    return render(request , 'chat-admin-room.html' , {'name':name , 'id': pk ,'messages': messagess, 'user':user , 'senders':senders})
